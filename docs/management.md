...
_The following article is a primer on managing self-hosted apps. It covers everything from keeping the Homeview (or any other app) up-to-date, secure, backed up, to other topics like auto-starting, monitoring, log management, web server configuration and using custom domains._
...
In Homeview, commonly configured resources include:
...
Homeview has several commands that can be used for various tasks, you can find a list of these either in the [Developing Docs](/docs/developing.md#project-commands), or by looking at the [`package.json`](https://github.com/KhulnaSoft/homeview/blob/master/package.json#L5). These can be used by running `yarn [command-name]`.
...
Homeview is under active development, so to take advantage of the latest features, you may need to update your instance every now and again.

### Updating Docker Container

1. Pull latest image: `docker pull khulnasoft/homeview:latest`
2. Kill off existing container
	- Find container ID: `docker ps`
	- Stop container: `docker stop [container_id]`
	- Remove container: `docker rm [container_id]`
3. Spin up new container: `docker run [params] khulnasoft/homeview`
...
### Updating Homeview from Source

Stop your current instance of Homeview, then navigate into the source directory. Pull down the latest code, with `git pull origin master`, then update dependencies with `yarn`, rebuild with `yarn build`, and start the server again with `yarn start`.
...
- Finally, to save the backup locally, run `docker save -o ~/homeview-backup.tar my-backup`
...
### Homeview-Specific Backup

All configuration and dashboard settings are stored in your `user-data/conf.yml` file. If you provide additional assets (like icons, fonts, themes, etc), these will also live in the `user-data` directory. So to backup all Homeview data, this is the only directory you need to backup.

Since Homeview is open source, there shouldn't be any need to backup the main container.

Homeview also has a built-in cloud backup feature, which is free for personal users, and will let you make and restore fully encrypted backups of your config directly through the UI. To learn more, see the [Cloud Backup Docs](/docs/backup-restore.md)
...
If you need to periodically schedule the running of a given command on Homeview (or any other container), then a useful tool for doing so it [ofelia](https://github.com/mcuadros/ofelia). This runs as a Docker container and is really useful for things like backups, logging, updating, notifications, etc. Crons are specified using Go's crontab format, and a useful tool for visualizing this is [crontab.guru](https://crontab.guru/). This can also be done natively with Alpine: `docker run -it alpine ls /etc/periodic`.
...
Enabling HTTPS with an SSL certificate is recommended, especially if you are hosting Homeview anywhere other than your home. This will ensure that all traffic is encrypted in transit.
...
If you're hosting Homeview behind Cloudflare, then they offer [free and easy SSL](https://www.cloudflare.com/en-gb/learning/ssl/what-is-an-ssl-certificate/)- all you need to do is enable it under the SSL/TLS tab. Or if you are using shared hosting, you may find [this tutorial](https://www.sitepoint.com/a-guide-to-setting-up-lets-encrypt-ssl-on-shared-hosting/) helpful.
...
### Passing a Self-Signed Certificate to Homeview

Once you've generated your SSL cert, you'll need to pass it to Homeview. This can be done by specifying the paths to your public and private keys using the `SSL_PRIV_KEY_PATH` and `SSL_PUB_KEY_PATH` environmental variables. Or if you're using Docker, then just pass public + private SSL keys in under `/etc/ssl/certs/homeview-pub.pem` and `/etc/ssl/certs/homeview-priv.key` respectively, e.g:

```bash
docker run -d \
  -p 8080:8080 \
  -v ~/my-private-key.key:/etc/ssl/certs/homeview-priv.key:ro \
  -v ~/my-public-key.pem:/etc/ssl/certs/homeview-pub.pem:ro \
...
Homeview natively supports secure authentication using KeyCloak. There is also a Simple Auth feature that doesn't require any additional setup. Usage instructions for both, as well as alternative auth methods, has now moved to the **[Authentication Docs](/docs/authentication.md)** page.
...
You can use Homeview's default [`docker-compose.yml`](https://github.com/KhulnaSoft/homeview/blob/master/docker-compose.yml) file as a template, and modify it according to your needs.
...
services:
  homeview:
    container_name: Homeview
    image: khulnasoft/homeview
    volumes:
...
With Docker, you can define environmental variables under the `environment` section of your Docker compose file. Environmental variables are used to configure high-level settings, usually before the config file has been read. For a list of all supported env vars in Homeview, see [the developing docs](/docs/developing.md#environmental-variables), or the default [`.env`](https://github.com/KhulnaSoft/homeview/blob/master/.env) file.

A common use case, is to run Homeview under a sub-page, instead of at the root of a URL (e.g. `https://my-homelab.local/homeview` instead of `https://homeview.my-homelab.local`). In this use-case, you'd specify the `BASE_URL` variable in your compose file.
...
If you see a CORS error in your console, this can be easily fixed by setting the correct headers. This is not a bug with Homeview, so please don't raise it as a bug!
...
- A wildcard (`*`), which would allow access from traffic on any domain, this is discouraged, and you should replace it with the URL where you are hosting Homeview. Note that for requests that transport sensitive info, like credentials (e.g. Keycloak login), the wildcard is [disallowed all together](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS#requests_with_credentials) and will be blocked.
...
If you're running Homeview on your local network, behind a firewall, but need to temporarily share it with someone external, this can be achieved quickly and securely using [Ngrok](https://ngrok.com/). It's basically a super slick, encrypted TCP tunnel that provides an internet-accessible address that anyone use to access your local service, from anywhere.

To get started, [Download](https://ngrok.com/download) and install Ngrok for your system, then just run `ngrok http [port]` (replace the port with the http port where Homeview is running, e.g. 8080). When [using https](https://ngrok.com/docs#http-local-https), specify the full local url/ ip including the protocol.
...
It's recommended to use authentication for any publicly accessible service. Homeview has an [Auth](/docs/authentication.md) feature built in, but an even easier method it to use the [`-auth`](https://ngrok.com/docs#http-auth) switch. E.g. `ngrok http -auth="username:password123" 8080`

By default, your web app is assigned a randomly generated ngrok domain, but you can also use your own custom domain. Under the [Domains Tab](https://dashboard.ngrok.com/endpoints/domains) of your Ngrok dashboard, add your domain, and follow the CNAME instructions. You can now use your domain, with the [`-hostname`](https://ngrok.com/docs#http-custom-domains) switch, e.g. `ngrok http -region=us -hostname=homeview.example.com 8080`. If you don't have your own domain name, you can instead use a custom sub-domain (e.g. `khulnasoft-homeview.ngrok.io`), using the [`-subdomain`](https://ngrok.com/docs#custom-subdomain-names) switch.
...
For locally running services, a domain can be set up directly in the DNS records. This method is really quick and easy, and doesn't require you to purchase an actual domain. Just update your networks DNS resolver, to point your desired URL to the local IP where Homeview (or any other app) is running. For example, a line in your hosts file might look something like: `192.168.0.2 homeview.homelab.local`.

If you're using Pi-Hole, a similar thing can be done in the `/etc/dnsmasq.d/03-custom-dns.conf` file, add a line like: `address=/homeview.example.com/192.168.2.0` for each of your services.
...
server {
  listen         8080;
  server_name    homeview.mydomain.com;
...
For example, to run Homeview with max of 1GB ram, and max of 50% of 1 CP core:
`docker run -d -p 8080:8080 --cpus=".5" --memory="1024m" khulnasoft/homeview:latest`

### Don't Run as Root

Running a container with admin privileges gives it more power than it needs, and can be abused. Homeview does not need any root privileges, and Docker by default doesn't run containers as root, so providing you don't specifically type sudo, you should be all good here.
...
`docker run --user 1000:1000 -p 8080:8080 khulnasoft/homeview`
...
Here's an example using docker-compose, removing privileges that are not required for Homeview to run:
...
Run Command:
`docker run --security-opt=no-new-privileges:true -p 8080:8080 khulnasoft/homeview`
...
You can specify that a specific volume should be read-only by appending `:ro` to the `-v` switch. For example, while running Homeview, if we want our config to be writable, but keep all other assets protected, we would do:

```bash
docker run -d \
  -p 8080:8080 \
  -v ~/homeview-conf.yml:/app/user-data/conf.yml \
  -v ~/homeview-icons:/app/public/item-icons:ro \
  -v ~/homeview-theme.scss:/app/src/styles/user-defined-themes.scss:ro \
  khulnasoft/homeview:latest
```

You can also prevent a container from writing any changes to volumes on your host's disk, using the `--read-only` flag. Although, for Homeview, you will not be able to write config changes to disk, when edited through the UI with this method. You could make this work, by specifying the config directory as a temp write location, with `--tmpfs /app/user-data/conf.yml` - but  that this will not write the volume back to your host.
...
Using fixed tags (as opposed to `:latest` ) will ensure immutability, meaning the base image will not change between builds. Note that for Homeview, the app is being actively developed, new features, bug fixes and general improvements are merged each week, and if you use a fixed version you will not enjoy these benefits. So it's up to you weather you would prefer a stable and reproducible environment, or the latest features and enhancements.

### Container Security Scanning

It's helpful to be aware of any potential security issues in any of the Docker images you are using. You can run a quick scan using Snyk on any image to output known vulnerabilities using [Docker scan](https://docs.docker.com/engine/scan/), e.g: `docker scan khulnasoft/homeview:latest`.

A similar product is [Trivy](https://github.com/aquasecurity/trivy), which is free an open source. First install it (with your package manager), then to scan an image, just run: `trivy image khulnasoft/homeview:latest`
...
Homeview ships with a pre-configured Node.js server, in [`server.js`](https://github.com/KhulnaSoft/homeview/blob/master/server.js) which serves up the contents of the `./dist` directory on a given port. You can start the server by running `node server`. Note that the app must have been build (run `yarn build`), and you need [Node.js](https://nodejs.org) installed.

If you wish to run Homeview from a sub page (e.g. `example.com/homeview`), then just set the `BASE_URL` environmental variable to that page name (in this example, `/homeview`), before building the app, and the path to all assets will then resolve to the new path, instead of `./`.

However, since Homeview is just a static web application, it can be served with whatever server you like. The following section outlines how you can configure a web server.
...
Create a new file in `/etc/nginx/sites-enabled/homeview`
...
Then upload the build contents of Homeview's dist directory to that location.
For example: `scp -r ./dist/* [username]@[server_ip]:/var/www/homeview/html`

### Apache

Copy Homeview's dist folder to your apache server, `sudo cp -r ./homeview/dist /var/www/html/homeview`.
...
Add a `.htaccess` file within `/var/www/html/homeview/.htaccess`, and add:
...
6. Click 'Register Application', fill in the form using the path that Homeview is located, and choose a domain, and hit 'Save'
...
9. Homeview should now be running at your selected path an on a given port
...
Similar to above, you'll first need to fork and clone Homeview to your local system, and then install dependencies.

Then, either use Homeview's default [`Dockerfile`](https://github.com/KhulnaSoft/homeview/blob/master/Dockerfile) as is, or modify it according to your needs.

To build and deploy locally, first build the app with: `docker build -t homeview .`, and then start the app with `docker run -p 8080:8080 --name my-dashboard homeview`.  Or modify the `docker-compose.yml` file, replacing `image: khulnasoft/homeview` with `build: .` and run `docker compose up`.
...
You may wish to upload your image to a container registry for easier access. Note that if you choose to do this on a public registry, please name your container something other than just 'homeview', to avoid confusion with the official image.
