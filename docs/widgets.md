...
Homeview has support for displaying dynamic content in the form of widgets. There are several built-in widgets available out-of-the-box as well as support for custom widgets to display stats from almost any service with an API.
...
You can also store images within Homeview's public directory (using a Docker volume), and reference them directly. E.g. `-v ./path/to/my-homelab-logo.png:/app/public/logo.png`, then in the widget `imagePath: /logo.png`.

Similarly, any web service that serves up widgets as image can be used. E.g. you could show current star chart for a GitHub repo, with: `imagePath: https://starchart.cc/KhulnaSoft/homeview.svg`.
...
- type: image
  options:
    imagePath: https://i.ibb.co/yhbt6CY/homeview.png
```
...
**Field** | **Type** | **Required** | **Description**
--- | --- | --- | ---
**`ipAddress`** | `string` |  _Optional_ | The IP to check. This can also be a domain/ host name or even an email address. If left blank, Homeview will use your current public IP address.
...
Display your coding summary. [Code::Stats](https://codestats.net/) is a free and open source app that aggregates statistics about your programming activity. Homeview supports both the public instance, as well as self-hosted versions.
...
Note, config for this widget is case-sensetive (see [#1268](https://github.com/KhulnaSoft/homeview/issues/1268))
...
**Field** | **Type** | **Required** | **Description**
--- | --- | --- | ---
**`username`** | `string` |  Required | The GitHub username to fetch info for. E.g. `khulnasoft`. (Not required if `hideProfileCard` and `hideLanguagesCard` are both set to `true`)
...
**`repos`** | `array` |  _Optional_ | If you'd like to also display stats for some GitHub repositories, then add an array or repo names here. Specified as `[username]/[repo-name]`, e.g. `khulnasoft/homeview`
...
- type: github-profile-stats
  options:
    ...
    repos:
    - khulnasoft/homeview
    - khulnasoft/personal-security-checklist
    - khulnasoft/twitter-sentiment-visualisation
```
...
This widget allows searching multiple search engines from homeview.
```yaml
  - type: 'custom-search'
    options:
...
Displays info about the server which Homeview is hosted on. Includes user + host, operating system, uptime and basic memory & load data.
...
Note that this widget is not available if you are running Homeview in a container or VM. Instead you can use the [System Monitoring](#system-resource-monitoring) widgets to display stats from the host system instead.
...
**`user_name`** | `string` |  Required | A Proxmox API Username, for example `root@pam` or `homeview@pve`.
...
- **500 Error in production mode**: Try adding the certificate authority (CA) certificate of your Proxmox host to Node.js. 
  - Download the Proxmox CA certificate to your Homeview host.
...
### Glances
The easiest method for displaying system info and resource usage in Homeview is with [Glances](https://nicolargo.github.io/glances/).
...
Many websites and apps provide their own embeddable widgets. These can be used with Homeview using the Embed widget, which lets you dynamically embed and HTML, CSS or JavaScript contents.
...
- type: embed
  options:
    html: '<img src="https://homeview.lan/item-icons/my-image.png" />'
```
...
Homeview has a built-in CORS proxy ([`services/cors-proxy.js`](https://github.com/KhulnaSoft/homeview/blob/master/services/cors-proxy.js)), which will be used automatically by some widgets, or can be forced to use by other by setting the `useProxy` option.
...
Alternatively, and more securely, you can set the auth headers on your service to accept requests from Homeview. For example:
...
Like elsewhere in Homeview, all colours can be easily modified with CSS variables.
...
For a full tutorial on creating your own widget, you can follow [this guide](/docs/development-guides.md/#building-a-widget), or take a look at [here](https://github.com/KhulnaSoft/homeview/commit/3da76ce2999f57f76a97454c0276301e39957b8e) for a code example.
...
The most common issue is a CORS error. This is a browser security mechanism which prevents the client-side app (Homeview) from from accessing resources on a remote origin, without that server's explicit permission (e.g. with headers like Access-Control-Allow-Origin). See the MDN Docs for more info: [Cross-Origin Resource Sharing](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS).
...
You can route requests through Homeview's built-in CORS proxy. Instructions and more details can be found [here](#proxying-requests). If you don't have control over the target origin, and you are running Homeview either through Docker, with the Node server or on Netlify, then this solution will work for you.
