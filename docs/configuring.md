...
**`statusCheck`** | `boolean` | _Optional_ | When set to `true`, Homeview will ping the URL associated with the current service, and display its status as a dot next to the item. The value here will override `appConfig.statusCheck` so you can turn off or on checks for a given service. Defaults to `appConfig.statusCheck`, falls back to `false`
