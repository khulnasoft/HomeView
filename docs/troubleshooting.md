...
If you're running Homeview using a static hosting provider (like Vercel), then there is no Node server, and so the save config action will not work via the UI.
...
Look here for more information: [https://homeview.to/docs/deployment#deploy-to-cloud-service](https://homeview.to/docs/deployment#deploy-to-cloud-service)
...
See also [#1465](https://github.com/KhulnaSoft/homeview/issues/1465)
...
This is not an issue with Homeview, but instead caused by the target app preventing direct access through embedded elements.
...
```text
Header set X-Frame-Options: "ALLOW-FROM http://[homeview-location]/"
```

### LightHttpd

```text
Content-Security-Policy: frame-ancestors 'self' https://[homeview-location]/
```
...
If you're seeing Homeview's 404 page on initial load/ refresh, and then the main app when you go back to Home, then this is likely caused by the Vue router, and if so can be fixed in one of two ways.
...
See also: [#628](https://github.com/KhulnaSoft/homeview/issues/628), [#762](https://github.com/KhulnaSoft/homeview/issues/762)
...
See also: [#670](https://github.com/KhulnaSoft/homeview/issues/670), [#763](https://github.com/KhulnaSoft/homeview/issues/763)

---

## Yarn Error

For more info, see [Issue #1](https://github.com/KhulnaSoft/homeview/issues/1)
...
- Try using [Docker](https://www.docker.com/get-started) instead, and all of the system setup and dependencies will already be taken care of. So from within the directory, just run `docker build -t khulnasoft/homeview .` to build, and then use docker start to run the project, e.g: `docker run -it -p 8080:8080 khulnasoft/homeview` (see the [deploying docs](https://github.com/KhulnaSoft/homeview/blob/master/docs/deployment.md#deploy-with-docker) for more info)
...
```text
Access to XMLHttpRequest at 'https://example.com/raw/my-config.yml' from origin 'http://homeview.local' has been blocked by CORS policy:
No 'Access-Control-Allow-Origin' header is present on the requested resource.
```

The solution is to add the appropriate headers onto the target server, to allow it to accept requests from the origin where you're running Homeview.
...
See also: [#380](https://github.com/KhulnaSoft/homeview/issues/380), [#350](https://github.com/KhulnaSoft/homeview/issues/350), [#297](https://github.com/KhulnaSoft/homeview/issues/297), [#349](https://github.com/KhulnaSoft/homeview/issues/349), [#510](https://github.com/KhulnaSoft/homeview/issues/510), [#511](https://github.com/KhulnaSoft/homeview/issues/511) and [#834](https://github.com/KhulnaSoft/homeview/issues/834)
...
See also [#624](https://github.com/KhulnaSoft/homeview/issues/624)
...
For more info, see [this announcement](https://github.com/KhulnaSoft/homeview/discussions/177).
...
```yaml
volumes:
- /srv/homeview/conf.yml:/app/user-data/conf.yml
- /srv/homeview/item-icons:/app/public/item-icons
```
...
You need to allow Homeview to make requests to Keycloak, and Keycloak to redirect to Homeview. The way you do this depends on how you're hosting these applications / which proxy you are using, and examples can be found in the [Management Docs](/docs/management.md#setting-headers).

For example, add the access control header to Keycloak, like:

`Access-Control-Allow-Origin [URL-of Homeview]`
...
```text
nginx.ingress.kubernetes.io/cors-allow-origin: "https://homeview.example.com"
...
If you find that your styles and other visual assets work when visiting `ip:port` by not `homeview.domain.com`, then this is usually caused by caching. In your browser, do a hard-refresh (<kbd>Ctrl</kbd> + <kbd>F5</kbd>). If you use Cloudflare, then you can clear the cache through the management console, or set the cache level to Bypass for certain files, under the Rules tab.
...
```text
Continuing execution. Pulling image khulnasoft/homeview:release-1.6.0
...
- Homeview is also available through GHCR, which at present does not have any hard limits. Just use `docker pull ghcr.io/khulnasoft/homeview:latest` to fetch the image
- You can also build the image from source, by cloning the repo, and running `docker build -t homeview .` or use the pre-made docker compose
...
The configuration file is validated against [Homeview's Schema](https://github.com/KhulnaSoft/homeview/blob/master/src/utils/ConfigSchema.json) using AJV.

First, check that your syntax is valid, using [YAML Validator](https://codebeautify.org/yaml-validator/) or [JSON Validator](https://codebeautify.org/jsonvalidator). If the issue persists, then take a look at the [schema](https://github.com/KhulnaSoft/homeview/blob/master/src/utils/ConfigSchema.json), and verify that the field you are trying to add/ modify matches the required format. You can also use [this tool](https://www.jsonschemavalidator.net/s/JFUj7X9J) to validate your JSON config against the schema, or run `yarn validate-config`.

If you're trying to use a recently released feature, and are getting a warning, this is likely because you've not yet updated the the current latest version of Homeview.
...
See also: [#776](https://github.com/KhulnaSoft/homeview/issues/776)
...
The most common widget issue is a CORS error. This is a browser security mechanism which prevents the client-side app (Homeview) from from accessing resources on a remote origin, without that server's explicit permission (e.g. with headers like Access-Control-Allow-Origin). See the MDN Docs for more info: [Cross-Origin Resource Sharing](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS).
...
Add the `Access-Control-Allow-Origin` header, with the value of either `*` to allow requests from anywhere, or more securely, the host of where Homeview is served from. For example:

```text
Access-Control-Allow-Origin: https://url-of-homeview.local
```
...
You can route requests through Homeview's built-in CORS proxy. Instructions and more details can be found [here](/docs/widgets.md#proxying-requests). If you don't have control over the target origin, and you are running Homeview either through Docker, with the Node server or on Netlify, then this solution will work for you.
...
A 401 error means your API key is invalid, it is not an issue with Homeview.
...
See also: [#803](https://github.com/KhulnaSoft/homeview/issues/803), [#789](https://github.com/KhulnaSoft/homeview/issues/789), [#577](https://github.com/KhulnaSoft/homeview/issues/577), [#621](https://github.com/KhulnaSoft/homeview/issues/621), [#578](https://github.com/KhulnaSoft/homeview/issues/578), [#806](https://github.com/KhulnaSoft/homeview/discussions/806)
...
If the raw API output is correct, yet the widget is rendering incorrect results, then it is likely a bug, and a ticket should be raised. You can start to debug the issue, by looking at the widget's code ([here](https://github.com/KhulnaSoft/homeview/tree/master/src/components/Widgets)), and the browser console + networking tab.

If the API itself is returning incorrect, incomplete or inaccurate data then an issue needs to be raised **with the API provider** (not Homeview!). You can find the API provider included within the widget docs, or for a full list see the [Privacy Docs](https://github.com/KhulnaSoft/homeview/blob/master/docs/privacy.md#widgets).

See also: [#807](https://github.com/KhulnaSoft/homeview/issues/807) (re, domain monitor)
...
- Or setup SSL - [here's a guide](https://github.com/KhulnaSoft/homeview/blob/master/docs/management.md#ssl-certificates) on doing so
...
You can also view any and all data that Homeview is storing, using the developer tools. Open your browser's dev tools (usually <kbd>F12</kbd>), in Chromium head to the Application tab, or in Firefox go to the Storage tab. Select Local Storage, then scroll down the the URL Homeview is running on. You should now see all data being stored, and you can select and delete any fields you wish.
...
You will need a GitHub account in order to raise a ticket. You can then [click here](https://github.com/KhulnaSoft/homeview/issues/new?assignees=khulnasoft&labels=%F0%9F%90%9B+Bug&template=bug.yml&title=%5BBUG%5D+%3Ctitle%3E) to open a new bug report.
...
- The issue is not covered in the [troubleshooting guide](https://github.com/KhulnaSoft/homeview/blob/master/docs/troubleshooting.md) or [docs](https://github.com/KhulnaSoft/homeview/tree/master/docs#readme)
...
Complete the fields, asking for your environment info and version of Homeview.
...
Finally, be sure to remain respectful to other users and project maintainers, in line with the [Contributor Covenant Code of Conduct](https://github.com/KhulnaSoft/homeview/blob/master/.github/CODE_OF_CONDUCT.md#contributor-covenant-code-of-conduct).
...
If you've contributed to Homeview (or any other project), but your contributions are not showing up on your GH profile, or in Homeview's [Credits Page](https://github.com/KhulnaSoft/homeview/blob/master/docs/credits.md), then this is likely a git config issue.
